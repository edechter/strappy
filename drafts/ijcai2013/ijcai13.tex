...
to the runs with larger frontier sizes is due to simply an increase in
the number of expressions seen, rather than increased learning, we should
see similarly good performance from this baseline run. In fact,
...
can test this hypothesis by varying the set of tasks to which the EC
algorithm is exposed. If it is true, then we should see a nonlinear
reponse to reducing the number of easy tasks, as the curriculum
...
\caption{Some learned primitives from the circuit based Boolean
  ...
  grammer. Note how $E_1$ can be used to define FALSE by taking the
...
EC algorithm's ability to learn Boolean functions using only the logical
...
that the representation the EC algorithm is using allows it to build
...
problem. There are two kinds of learning that the EC algorithm might
...
Figure~\ref{fig:boolLearningCurvesUniform}, we show 10 runs of the EC
algorithm on the second experiment with a frontier size of $2000$: note how there are several
local minima that the majority of the runs get stuck in with performance
around $50\%$, but several of the runs seem to take different
trajectories to more successful representations.
...
here is not that the EC algorithm is optimal, but that it captures key
...
reuses succcessful patterns of reasoning that span many problems in a
domain.

At their best, human processes of discovery also clearly go beyond
...
in similar ways, but at least sometimes they can realize that they are
...
problems it is ready to tackle when, which problems are appropriate in
difficulty given what it currently knows.  Yet people can sometimes
