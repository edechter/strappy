...
\li \id{suffixWeight}, \id{suffixRewards} = \func{Sample-Plan}$(\rho e)$
\li \Else
\li \id{suffixWeight}, \id{suffixRewards} = $0$, $\emptyset$
\li \End
\li \Comment{To satisfy the boundary conditions of the recurrence, $P(t|\emptyset)$ is defined to be 1.}
\li \id{eReward} = $\frac{1+\mbox{\id{suffixRewards}}}{P(t|\rho)}$
\li \Return \id{eReward}, $\{(e, \mbox{\id{eReward}})\}\cup \mbox{\id{suffixRewards}}$
\end{codebox}
