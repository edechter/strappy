#!/usr/bin/env python
# EX2: Boolean circuits EC2 experiments. 
####################
...
def send_ex2_job(inst):
    script = """
cd ~/
rm -fr program-induction
git clone git@bitbucket.org:edechter/program-induction.git
cd ~/program-induction/src/PI/
ghc --make -O2 -rtsopts  -main-is Experiments.EX2.main -o runEX2 Experiments/EX2.hs 
./runEX2 
git pull
git add data/EX2*
git commit -m \'Data from ec2 instance %s running ex2 experiment.\'
git push
""" % (inst.id)
    ...
    logfilename = "./logs/ec2.ex2.%s.log" % inst.id
...
def send_ex2_jobs_when_ready(requests):
    ec2 = boto.connect_ec2()
    ...
    while True:
        ...
        for inst in instances:
            print "" 
            print "EX2 job sent to instance %s at dns %s" % (inst.id, inst.public_dns_name)
            send_ex2_job(inst)
            time.sleep(10)
...
def launch_spots_send_jobs(count=1):
    ...
    send_ex2_jobs_when_ready(requests)
    
    
if __name__ == "__main__":
    launch_spots_send_jobs(count=2)
